...
layout (rgba32f, binding = 2) uniform sampler2D u_HeirarchyTree;
...
bool TestIntersectAABB(vec3 minimum, vec3 maximum, Ray ray){
...
int node_ID_stack[stack_capacity];
int node_ID_stack_size = 0;
void push_node_ID(int ID){
	if(node_ID_stack_size < stack_capacity){
		node_ID_stack[node_ID_stack_size] = ID;
		node_ID_stack_size++;
	}
}
int pop_out_node_ID(){
	if(node_ID_stack_size > 0){
		node_ID_stack_size--;
		return node_ID_stack[node_ID_stack_size];
	}
	return -1;
}
...
int FindNextIntersectionCandidate(Ray ray){
	...
	// only inserted if intersected
	if (node_ID_stack_size == 0){
		push_node_ID(0); // root
	}
	while(node_ID_stack_size > 0){
		...
		if(TestIntersectAABB(BBmin, BBmax, ray)){
			if(left != 0 && right != 0){ // Internal node
				push_node_ID(right);
				push_node_ID(left);
			}
			else return int(texture(u_HeirarchyTree, vec2(2.1 / node_data_size, (ID + 0.1) / node_tree_size)).x);
		}
...
/*float TestIntersectionWithObj(vec3 origin, vec3 dirn, int ObjectID, out vec3 transformed_ray_dirn){
	int Type = int(texture(u_ObjGroupIDTexture, vec2((ObjectID + 0.1) / id_tex_size_x, 0.1)).r);					   // texture fetch is expensive
	vec3 position      = texture(u_ObjGroupDataTexture, vec2(0.1 / data_tex_size.x, (ObjectID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
	vec3 last_position = texture(u_ObjGroupDataTexture, vec2(1.1 / data_tex_size.x, (ObjectID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
	matrix[0]          = texture(u_ObjGroupDataTexture, vec2(2.1 / data_tex_size.x, (ObjectID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
	matrix[1]          = texture(u_ObjGroupDataTexture, vec2(3.1 / data_tex_size.x, (ObjectID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
	matrix[2]          = texture(u_ObjGroupDataTexture, vec2(4.1 / data_tex_size.x, (ObjectID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
	vec3 scale         = texture(u_ObjGroupDataTexture, vec2(5.1 / data_tex_size.x, (ObjectID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
...
RayReturnData LaunchRay(vec3 g_origin, vec3 g_dirn, float max_t_depth, float color_contribution, int sample_index, int max_samples){
    ...
    while(IntersectedObjID > -1){
		...
		vec3 position, scale; // vec3 last_position;
		mat3 matrix;
		int Type = int(texture(u_ObjGroupIDTexture, vec2((IntersectedObjID + 0.1) / id_tex_size_x, 0.1)).r);					   // texture fetch is expensive
		position      = texture(u_ObjGroupDataTexture, vec2(0.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
	//	last_position = texture(u_ObjGroupDataTexture, vec2(1.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
		matrix[0]     = texture(u_ObjGroupDataTexture, vec2(2.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
		matrix[1]     = texture(u_ObjGroupDataTexture, vec2(3.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
		matrix[2]     = texture(u_ObjGroupDataTexture, vec2(4.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
		scale         = texture(u_ObjGroupDataTexture, vec2(5.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
		...
		if(min_t_depth > t && t > 0){
			...
			data.color          = texture(u_ObjGroupDataTexture, vec2(6.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
			data.material       = texture(u_ObjGroupDataTexture, vec2(7.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xyz;// texture fetch is expensive
			data.scatteritivity = texture(u_ObjGroupDataTexture, vec2(7.1 / data_tex_size.x, (IntersectedObjID + 0.1) / data_tex_size.y)).xy;// texture fetch is expensive
