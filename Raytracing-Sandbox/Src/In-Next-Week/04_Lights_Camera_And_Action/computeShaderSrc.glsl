...
struct LightObjIdentifier{
		float BBmin[3];
		float BBmax[3];
		uint GeomIdx;
	};
layout(std430) buffer sbo_Light{
	 LightObjIdentifier EmissiveObjects[];
};

uniform uint u_NumOfLightSources;
////// HELPER /////////////
vec3 Background_Color(vec3 ray_dirn) { // For sky or somthing like that

// I Don't know why but EmissiveObjects.length() == (actual EmissiveObjects.length() - 1) i.e actual EmissiveObjects.length() == 0 => EmissiveObjects.length() == 128 & actual EmissiveObjects.length() == 1 => EmissiveObjects.length() == 0 so-on,
	vec3 lowerGrad = u_NumOfLightSources > 0 ? vec3(0) : vec3 (1.0, 1.0, 1.0);
	vec3 upperGrad = u_NumOfLightSources > 0 ? vec3(0) : vec3 (0.3, 0.4, 1.0);
	float t = (ray_dirn.y + 1.0)*0.5;
	return ((1.0 - t)*lowerGrad + t*upperGrad);
}
...
bool IntersectRayMinimal(Ray ray, float GeomIndex, inout float t_limiting, inout float extraData){
...
bool IfInsideAABBAndLeaf_TryAccumulateRI(float NodeIndex, vec3 hitPoint, inout float accumulator, inout uint accumulator_counter, out float leftChildIDX){
	...
	{ // Accumulate
		if(leftData < 0.1){ // Leaf Node with objID ∈[0 -NumOfGeom)
			...
			switch(Type){
				case ELLIPSOID:
					isInside = int(dot(vector, vector) <= 1.0); break;
				case CUBOID:
					isInside = int(abs(vector.x) <= 0.5);
					isInside &= int(abs(vector.y) <= 0.5);
					isInside &= int(abs(vector.z) <= 0.5);
					break;
				default:
					isInside = 0;
			}
...
bool isLitGeom(uint Input){
	bool _return = false;
	for (uint i = 0; i < u_NumOfLightSources && !_return; i++){
		_return = Input == EmissiveObjects[i].GeomIdx;
	}
	return _return;
}
	...
	while (flt_Stack.size > 0)
	{
		...
		vec3 IncomingDirn; 
		{
			...
			vec3 localHitPosnOnGeom;
			{
				...
				//######### Shortcut to find Texture Index, maybe i'll change it to transformed-normal
...
//  ^--------This LOOP (Outer most)
			...
			uint is_lit = u_NumOfLightSources > 0 ? 0 : uint(isLitGeom(uint(final_GeomIndex + 0.1f)));

			if(is_lit == 0)
			{
				for (uint i = 0; i < u_NumOfLightSources; i++) {
					{
						LightObjIdentifier light_data = EmissiveObjects[i];
						vec3 BBmin = vec3(light_data.BBmin[0], light_data.BBmin[1], light_data.BBmin[2]);
						vec3 BBmax = vec3(light_data.BBmax[0], light_data.BBmax[1], light_data.BBmax[2]);
						// set currRay
						currRay.Origin = hitData.HitPoint + 0.0001*hitData.NormalAtHit;
						t_limiting = length((BBmax + BBmin)*0.5 - currRay.Origin) + length(BBmax - BBmin);
						
						currRay.Direction = normalize((BBmin + (BBmax - BBmin)*(float(gl_LocalInvocationIndex) / (gl_WorkGroupSize.x * gl_WorkGroupSize.y * gl_WorkGroupSize.z))) - currRay.Origin);
						final_GeomIndex = -1;
					}
					uint InitialSize = flt_Stack.size;
					STK_PUSH(flt_Stack, 0);
					...
					// Find Candidate
						...
						if(GeomIndex > -0.9) {
							// Material's Refractive Index
							float dummy;
							final_GeomIndex = IntersectRayMinimal(currRay, GeomIndex, t_limiting,  dummy) ? GeomIndex : final_GeomIndex;
						} else break;
					} while(true);
					is_lit += uint(isLitGeom(uint(final_GeomIndex + 0.1f)));
				}
				contribution *= (float(is_lit)/max(u_NumOfLightSources, 1));
			}
			else{
				final_color = vec3(1);
				return ;
