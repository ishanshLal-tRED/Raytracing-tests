...
namespace In_Next_Week
{
	...
	public:
		...
		inline bool OnImGuiRenderMin ()
		{
			return Helper::IMGUI::DrawVec3Control ("Position", position) 
				...
				| Helper::IMGUI::DrawVec3Control ("Scale", scale, 100.0f);
		...
		bool FillTransformBuff (Transform_Buff *buffer)
		{
			...
			buffer->DeltaPosition[0] = position[0] - last_position[0], buffer->DeltaPosition[1] = position[1] - last_position[1], buffer->DeltaPosition[2] = position[2] - last_position[2];
	...
	protected:
		...
		int m_NumberOfGeometriesToRender = 4;
		int m_NumberOfRayBounces = 5;
		...
		struct  
		{
			...
			float Aperture = 1.0f;
			int NumberOfFocusDist = 1;
			std::vector<float> FocusDists;
		...
		struct  
		{
			...
			GLuint NumberOfBouncesPerSampleRay = 0;
	...
	void In_Next_Week::RT_Base<GeometryData, GeometryBuff>::OnImGuiRenderBase ()
	{
		...
		if (ImGui::InputInt ("Number Of Bounces per Ray Sample", &m_NumberOfRayBounces)) {
			m_NumberOfRayBounces = MAX (0, m_NumberOfRayBounces);
			m_RedrawFrame = true;
		}
		...
		if (ImGui::CollapsingHeader ("Camera", nullptr, ImGuiTreeNodeFlags_DefaultOpen  | ImGuiTreeNodeFlags_SpanAvailWidth)) {
			...
			{
				ImGui::PushID (i);
				m_RedrawFrame |= ImGui::DragFloat ("focus", &m_Camera.FocusDists[i], 0.001f);
				ImGui::PopID ();
			}
	...
	void In_Next_Week::RT_Base<GeometryData, GeometryBuff>::OnComputeShaderReloadBase ()
	{
		...
		m_UniformID.NumberOfBouncesPerSampleRay = glGetUniformLocation (m_ComputeShaderProgID, "u_NumOfBounces");
		...
		m_UniformID.Camera.FocusDists_at_0 = glGetUniformLocation (m_ComputeShaderProgID, "u_Camera.FocusDist[0]"); // Subsequent are prev's_UniformID++
		m_UniformID.Camera.Aperture = glGetUniformLocation (m_ComputeShaderProgID, "u_Camera.Aperture");
		m_RedrawFrame = true;
	}
}