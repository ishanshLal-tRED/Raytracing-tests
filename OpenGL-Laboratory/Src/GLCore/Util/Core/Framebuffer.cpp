...
#include "glad-OpenGL_4.4/include/glad/glad.h"
...
namespace GLCore
{
	namespace Utils
	{
		...
		std::shared_ptr<GLCore::Utils::Framebuffer> Framebuffer::Create (const FramebufferSpecification &spec)
		{
		...
		namespace Utils
		{
			...
			{
				glGenTextures (count, outIDs);
				for (uint32_t i = 0; i < count; i++) {
					// glBindTextures(GLuint first, GLsizei count, const GLuint *textures);
					glBindTexture (TextureTarget (multisampled), outIDs[i]);
				}
